...
male_trt_0 <- male.full %>% as_tibble() %>%
  filter(widow == 0) %>%
  select(-widow, -hhidpn)
  
male_trt_1 <- male.full %>% as_tibble() %>%
  filter(widow == 1) %>%
  select(-widow, -hhidpn)
...
covariates <- male.full %>% select(birth_year , child_liv , migration , num_shared_child , child_dead
                                   ,  age_diff , mean_drink_days , mean_drink_num
                                   , mean_hosp_time , max_hosp_visit , mean_log_fam_income , max_pension , mean_bmi , max_nurs_home 
                                   , mean_cesd , mean_expend , max_high_bp , max_diabetes , max_cancer , max_lung , max_heart , max_stroke , max_psych ,eduyrs , white , black 
...
for (i in 1:B) {
  ...
  boot_sample <- male.full[sample(nrow(male.full), replace = TRUE), ]
...
ggplot(new_data, aes(x = ps.logit)) +
  ...
  geom_line(aes(y = survival), color = "blue") +
  geom_line(aes(y = mean_survival), color = "red") +
  labs(x = "Propensity Score", y = "Survival Probability", 
       title = "Survival Probability vs Propensity Score") +
  scale_color_manual(values = c("blue", "red"), 
