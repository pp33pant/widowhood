...
data_1 <- male_drop_na 
male_trt_0 <- data_1 %>% as_tibble() %>%
  filter(widow == 0) %>%
  select(-widow, -hhidpn)
  
male_trt_1 <- data_1 %>% as_tibble() %>%
  filter(widow == 1) %>%
  select(-widow, -hhidpn)
...
ci_mean <- cox_hte$hte
ps.logit <- seq(0, 0.98, length.out = 50)
ate <- cox_ate$ate
